...
func NewServerNetwork(
	...
	s.accept = accept
	s.closed = closed
	s.kv = keyval.NewKeyVal()                                //creates a new keyvalue store
	s.commandHandlers = make(map[string]func(Conn, Command)) //creates a map of command handlers
	s.SetupCommandHandlers()                                 //sets up the command handlers
	return s
}
...
func (s *Server) handleSET(conn Conn, cmd Command) {
	if len(cmd.Args) < 3 { // command format: SET key value, therefore there must be at least 3 arguments
		conn.WriteError("ERR wrong number of arguments for 'set' command")
		return
	}
	key := cmd.Args[1]          // Convert the second argument into a string to use as the key
	value := cmd.Args[2]        // The third argument is the value (byte slice)
	err := s.kv.Set(key, value) // Store the key-value pair using the key-value storage system
	if err != nil {
		conn.WriteError("ERR " + err.Error()) // On error, send an error message back to the client
	} else {
		conn.WriteString("OK") // On success, send "OK" to the client
	}
}

// handleGET handles the GET command
func (s *Server) handleGET(conn Conn, cmd Command) {
	if len(cmd.Args) < 2 { // command format: GET key
		conn.WriteError("ERR wrong number of arguments for 'get' command")
		return
	}
	key := cmd.Args[1]         // Convert the second argument into a string to use as the key
	value, ok := s.kv.Get(key) // Retrieve the value associated with the key
	if !ok {
		conn.WriteBulk(nil) // If the key does not exist, return nil (Redis protocol)
	} else {
		conn.WriteBulk(value) // If the key exists, return the value
	}
}
...
func (s *Server) handleCommand(conn Conn, cmd Command) {
	if handler, ok := s.commandHandlers[string(cmd.Args[0])]; ok {
		handler(conn, cmd)
	} else {
...
// conn represents a client connection
type conn struct {
	conn      net.Conn //responsible for actually transporting the data. The data is is wr (buffer)
	wr        *Writer
	rd        *Reader
	addr      string
	ctx       interface{}
...
type Server struct {
	...
	laddr           string
	handler         func(conn Conn, cmd Command)
	commandHandlers map[string]func(Conn, Command)
	accept          func(conn Conn) bool
	closed          func(conn Conn, err error)
	conns           map[*conn]bool
	ln              net.Listener
